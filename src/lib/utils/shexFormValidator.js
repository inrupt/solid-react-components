class ShexFormValidator {
  ...
  formStringValidation = field => {
    ...
    if (valueExpr.minlength || valueExpr.maxlength) {
      if (
        (valueExpr.minlength && valueExpr.minlength > field.value.length) ||
  ...
  formNumberValidation = field => {
    ...
    if (valueExpr) {
      if (valueExpr.mininclusive || valueExpr.maxinclusive) {
        if (
          (valueExpr.mininclusive && valueExpr.mininclusive > field.value) ||
      ...
      if (valueExpr.mininclusive || valueExpr.maxinclusive) {
        if (
          (valueExpr.minexclusive && valueExpr.minexclusive >= field.value) ||
...
export default ShexFormValidator;
