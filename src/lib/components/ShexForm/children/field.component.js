...
const Field = (props) => {
    ...
    return (<div>
        <label>{label}</label>
        <ul>{
            values.map((value,i) => <li key={i}>
                {inputType === 'text' ? <input type='text' value={value} /> : <select>{data.valueExpr.values.map(value => <option value={value}>{value.split("#")[1]}</option>)}</select>
                }
            </li>)
        }
        </ul>
